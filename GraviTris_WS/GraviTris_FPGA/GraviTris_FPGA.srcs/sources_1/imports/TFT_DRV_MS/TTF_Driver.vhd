...
architecture Behavioral of TTF_Driver is
  ...
  signal counter      : natural range 0 to 28810 := 0;
begin
          ...
          case stateIdx is
            ...
            when setupToPrintFrame_st => -- Get Width, Height and send 0x2C command to start write to memory
              ...
              if to_integer(unsigned(stepCount)) = 74 then --(4Mhz clk 4)
                ...
                address <= std_logic_vector(unsigned(address) + 1); -- skip to next 8 pixels in BRAM
            ...
            when printFrame_st =>
              ...
              if to_integer(unsigned(stepCount)) = 0 then -- Setup MSB pixel 0 (4Mhz clk 0) 
                ...
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
                -- ######## Pixels 3,4 ########
              elsif to_integer(unsigned(stepCount)) = 200 then -- Setup MSB pixle 3 (4Mhz clk 8.1)
                tftData_out <= pixel3(15 downto 8);
              elsif to_integer(unsigned(stepCount)) = 212 then -- Send MSB pixle 3 (4Mhz clk 8.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 224 then -- Reset WRX pin (4Mhz clk 9)
                WRX_bit <= '0';
              elsif to_integer(unsigned(stepCount)) = 225 then -- Setup LSB pixle 3 (4Mhz clk 9.1)
                tftData_out <= pixel3(7 downto 0);
              elsif to_integer(unsigned(stepCount)) = 236 then -- Send LSB pixle 3 (4Mhz clk 9.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 249 then -- Reset WRX pin (4Mhz clk 10)
                WRX_bit    <= '0';
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
              elsif to_integer(unsigned(stepCount)) = 250 then -- Setup MSB pixle 4 (4Mhz clk 10.1)
                tftData_out <= pixel4(15 downto 8);
              elsif to_integer(unsigned(stepCount)) = 262 then -- Send MSB pixle 4 (4Mhz clk 10.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 274 then -- Reset WRX pin (4Mhz clk 11)
                WRX_bit <= '0';
              elsif to_integer(unsigned(stepCount)) = 275 then -- Setup LSB pixle 4 (4Mhz clk 11.1)
                ...
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
                -- ######## Pixels 4,5 ########
              elsif to_integer(unsigned(stepCount)) = 300 then -- Setup MSB pixle 4 (4Mhz clk 13.1)
                tftData_out <= pixel4(15 downto 8);
              elsif to_integer(unsigned(stepCount)) = 312 then -- Send MSB pixle 4 (4Mhz clk 13.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 324 then -- Reset WRX pin (4Mhz clk 14)
                WRX_bit <= '0';
              elsif to_integer(unsigned(stepCount)) = 325 then -- Setup LSB pixle 4 (4Mhz clk 14.1)
                tftData_out <= pixel4(7 downto 0);
              elsif to_integer(unsigned(stepCount)) = 336 then -- Send LSB pixle 4 (4Mhz clk 14.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 349 then -- Reset WRX pin (4Mhz clk 15) 
                WRX_bit    <= '0';
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
              elsif to_integer(unsigned(stepCount)) = 350 then -- Setup MSB pixle 5 (4Mhz clk 15.1)
                tftData_out <= pixel5(15 downto 8);
              elsif to_integer(unsigned(stepCount)) = 362 then -- Send MSB pixle 5 (4Mhz clk 15.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 374 then -- Reset WRX pin (4Mhz clk 16)
                WRX_bit <= '0';
              elsif to_integer(unsigned(stepCount)) = 375 then -- Setup LSB pixle 5 (4Mhz clk 16.1)
                tftData_out <= pixel5(7 downto 0);
              elsif to_integer(unsigned(stepCount)) = 386 then -- Send LSB pixle 5 (4Mhz clk 16.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 399 then -- Reset WRX pin (4Mhz clk 17)
                WRX_bit    <= '0';
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
                -- ######## Pixels 6,7 ########
              elsif to_integer(unsigned(stepCount)) = 400 then -- Setup MSB pixle 6 (4Mhz clk 17.1)
                tftData_out <= pixel6(15 downto 8);
              elsif to_integer(unsigned(stepCount)) = 412 then -- Send MSB pixle 6 (4Mhz clk 17.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 424 then -- Reset WRX pin (4Mhz clk 18)
                WRX_bit <= '0';
              elsif to_integer(unsigned(stepCount)) = 425 then -- Setup LSB pixle 6 (4Mhz clk 18.1)
                tftData_out <= pixel6(7 downto 0);
              elsif to_integer(unsigned(stepCount)) = 436 then -- Send LSB pixle 6 (4Mhz clk 18.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 449 then -- Reset WRX pin  (4Mhz clk 19)
                WRX_bit    <= '0';
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
              elsif to_integer(unsigned(stepCount)) = 450 then -- Setup MSB pixle 7 (4Mhz clk 19.1)
                tftData_out <= pixel7(15 downto 8);
              elsif to_integer(unsigned(stepCount)) = 462 then -- Send MSB pixle 7 (4Mhz clk 19.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 474 then -- Reset WRX pin (4Mhz clk 20)
                WRX_bit <= '0';
              elsif to_integer(unsigned(stepCount)) = 475 then -- Setup LSB pixle 7 (4Mhz clk 20.1)
                tftData_out <= pixel7(7 downto 0);
              elsif to_integer(unsigned(stepCount)) = 486 then -- Send LSB pixle 7 (4Mhz clk 20.5)
                WRX_bit <= '1';
              elsif to_integer(unsigned(stepCount)) = 499 then -- Reset WRX pin (4Mhz clk 21)
                WRX_bit    <= '0';
                pixelCount <= std_logic_vector(unsigned(pixelCount) + 1);
              ...
              if to_integer(unsigned(stepCount)) = 499 then
